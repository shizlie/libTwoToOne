# PROTOCOL.md — Mesh Room Protocol (wire v1 · spec rev v1.8)
...
> Status: **AMENDED v1.8 — 2026-07-12** (Intent P room scalability: `system.checkpoint`
> performative, archive-to-R2 pruning, storage-pressure escalate, FTS/artifact
> size config, multipart artifact upload — see Changelog). Previously AMENDED
> v1.7 — 2026-07-11 (Intents K/L/M — authority truth, signal ownership,
...
- **v1.8 (2026-07-12)** — Intent P (room scalability) amendment: new room-authored
  `system.checkpoint` performative (`data:{head_seq:number, head_hash:string,
  snapshot_hash:string}`; `snapshot_hash` covers `{claims, roster, decisions}` —
  each array sorted ascending by its table's primary key (`claims` by `task_ref`,
  `roster` by `participant_id`, `decisions` by `id`; JCS canonicalizes object
  KEYS but never array element order, so the contract pins an explicit total
  order a verifier reproduces with its own sort) — LOG-DERIVED state projections only (claims omit `lease_expires`/
  `max_claim_until`/`claim_window_until` — heartbeat mutates these with no log
  entry, so a verifier replaying the log could never reproduce them; decisions
  omit `authority_holders` — a wall-clock role-binding read taken at emission
  time, never itself recorded), excluding `head` (already carried separately as
  `head_seq`/`head_hash`); `GET /snapshot`'s own `room_sig` payload is
  unchanged — still the FULL live view (`lease_expires`/`authority_holders`
  included) and stays head-inclusive) emitted when `archive.threshold_entries` (new `system.config` /
  `POST /config` field, 0/absent = off, bounds `[100, 1000000]`) is crossed, followed by archive-to-R2
  pruning of the covered entries range (`${room}/archive/${from}-${to}.jsonl`,
  tiering never deletion — `GET /entries`, `WS /stream` backfill, and `GET /verify` read through
  archived ranges transparently); `GET /entries` gains `notifies_truncated:true`
  when the archived-side notify derivation hits its 10k-entry budget (the
  `entries[]` page itself is unaffected); `WS /stream` backfill gains a
  `{type:"backfill_truncated", resume_since}` frame (§6) when the archived
  replay hits the same 10k-entry budget, then continues the live stream
  normally — a client that receives it MUST page `GET /entries` from
  `resume_since` up to the head it first receives, to fill the gap; `escalate`
  gains `data.reason:
  "storage_pressure"` (`{used_bytes, limit_bytes}`), checked every 256th
  append against the shared 1 GiB DO SQLite ceiling, at most once per
  80/90/95% band per 24h (re-fires only once `used_bytes` has grown since that
  band's last fire — SQLite freelist reuse after pruning means the size need
  not visibly shrink); `system.config` gains `fts_max_file_bytes` (default
  262144) and `artifact_max_bytes` (default 26214400), both exposed as
  effective values on `GET /state`'s `defaults`; `GET /search` response gains
  `skipped_count` (files excluded from FTS by `fts_max_file_bytes`, tracked in
  a new durable `file_fts_skipped` projection — never silently absent); `PUT
  /artifacts/:hash` promotes to R2 native multipart the moment the ACTUAL
  streamed byte count (never the client-declared, possibly-lying
  `Content-Length`) crosses one 10 MiB part, replacing the fixed 25 MiB
  buffered-in-DO-memory wall with the room-configured `artifact_max_bytes`
  cap (checked against the declared `Content-Length` up front AND the
  running streamed total throughout — a lying/absent header can no longer
  force unbounded buffering);
  `/tree` rows gain `tip_ts`
  (derived at read via a `entries.room_ts` join on `tip_seq`, materialized
  into `file_tree` only at prune time so it survives archival).
...
| `escalate` | **room only** (†`timer_failed`/`storage_pressure` escalates omit `task_ref`) | required† | `data.reason`: `unclaimed_timeout` \| `lease_expired` \| `stalled` \| `timer_failed` \| `storage_pressure` (`{used_bytes, limit_bytes}`, Intent P S-P4; `contested` reserved for v2); may carry `{last_condition, last_condition_seq}` (Intent F); **room-authored escalates carry `data.to: string[]`** — the room-authored default target (`[owner, ...verdict_by]` deduped for task escalates; `[owner]` for task-less `timer_failed`/`storage_pressure`). `data.to` is advisory routing ONLY: a daemon MAY use it as one signal among many (mentions, watches, derived interest) for wake selection — the room stays neutral (Intent C). Every escalate entry also folds an OPEN row into the `escalations` projection (Intent L, below) |
...
| `system.config` | **room only** | — | data: `{default_access?:"open"\|"closed"\|{discover,write}, rate_limit?:string, authority_source?:"card"\|"bindings", archive?:{threshold_entries?:number}, fts_max_file_bytes?:number, artifact_max_bytes?:number}` (at least one field required); owner-initiated via `POST /config`; `default_access` sets the room's write/discover posture — a legacy single string is normalized to both grades on read (S-K5 wire compat: an old CLI posting the legacy string is never rejected — flip is immediate, see R-E8); `authority_source` sets the verdict-resolution posture (`card` = self-declared `card.roles`, `bindings` = owner-managed `role_bindings`, S-K1/S-K2) and/or the room's per-participant `rate_limit` (§7); a live `rate_limit` update re-seeds the DO's `RateLimiter` on the next append (in-flight token buckets reset — intended, so a raise takes effect at once, not after the old window drains); `rate_limit` must parse as `"<rate>/min[;burst=<n>]"` and satisfy the §7 bounds, else `400 invalid_submission` (same code as genesis `defaults.rate_limit`, distinct message per failure mode); `archive.threshold_entries` (Intent P, 0/absent=off, bounds `[100,1000000]`) is the entries-axis checkpoint trigger (§4.1, `system.checkpoint` below); `fts_max_file_bytes` (Intent P, default 262144, bounds `[1024, 5242880]`) is the per-file FTS indexing size cap; `artifact_max_bytes` (Intent P, default 26214400, bounds `[1048576, 524288000]`) is the room's artifact-upload cap (`PUT /artifacts/:hash`, §8); all three Intent P fields are exposed as effective values on `GET /state`'s `defaults` even before the owner ever calls `POST /config` (S-P5) |
...
| `system.checkpoint` | **room only** | — | data: `{head_seq:number, head_hash:string, snapshot_hash:string}`; room-authored, signed, when `archive.threshold_entries` is crossed since the last checkpoint (Intent P §4.1); `head_hash` is the `entry_hash` of the entry at `head_seq`; `snapshot_hash` is `"sha256:"+sha256Hex(jcs({claims,roster,decisions}))` with each array sorted ascending by its primary key (`claims` by `task_ref`, `roster` by `participant_id`, `decisions` by `id` — JCS never orders array elements, so the sort IS part of the contract) over LOG-DERIVED projections ONLY — `claims` omit `lease_expires`/`max_claim_until`/`claim_window_until` (heartbeat-mutated, no log trace) and `decisions` omit `authority_holders` (a wall-clock role-binding read at emission time, never recorded) — so every checkpoint is reproducible by a verifier replaying the log to `head_seq`, unlike `GET /snapshot`'s FULL live view (which keeps both fields and is a separate, unchanged, head-inclusive signature); excludes `head` (already carried above as `head_seq`/`head_hash`); followed by archive-to-R2 pruning of the range up to and including `head_seq` — tiering, never deletion (S-P2) |
...
- WS frames: `{type:"entry", entry}` · `{type:"notify", watch_id, entry_seq}` ·
  `{type:"ping"}` · `{type:"backfill_truncated", resume_since}` (Intent P, v1.8 —
  sent once, after a WS backfill's archived-replay budget is hit, then the live
  stream continues normally; a client receiving it MUST page `GET
  /entries?since=resume_since` up to the head it first receives, to fill the
  gap). Poll responses carry the same data (§8).
...
| `GET /v1/rooms/:room/entries?since=&limit=&wait_s=&performative=&exclude=` | entries after `seq=since` (≤ limit, default 100). `wait_s` ≤ 55 long-polls until data or timeout → `{entries[], notifies[], head}`. `performative=`/`exclude=` (CSV, exact match or `file.*`/`system.*` prefix wildcards, ≤ 50 items each) narrow the set server-side. Transparently reads through an archived range (Intent P, `system.checkpoint` pruning) — the caller never observes a seq gap within its declared retention; the archived-side notify derivation covers exactly `(since, since+10000]` — contiguous from the caller's OWN `since`, never a detached window — and the response additionally carries `notifies_truncated:true` when that 10k-entry budget is hit (`entries[]` paging is unaffected). A client that pages `GET /entries` forward, advancing `since` after every response as it already must, therefore reaches full notify coverage with zero new wire surface: keep paging until a response carries no `notifies_truncated` |
| `GET /v1/rooms/:room/tree?prefix=` | membership/discover-gated file metadata tree; each row `{path, content_hash?, size, tip_seq, tip_ts?, lease_holder?}` — `tip_ts` (Intent P, v1.8) is the tip entry's `room_ts`, derived at read, present regardless of `read` grade (metadata, not content) |
| `GET /v1/rooms/:room/search?q=&prefix=&limit=` | FTS5 full-text search over indexed file content → `{results:[{path,snippet,score}], skipped_count}` — `skipped_count` (Intent P, v1.8) counts paths excluded from the index by `fts_max_file_bytes` (matching `prefix` when given) |
| `PUT /v1/rooms/:room/artifacts/:hash` | content-addressed blob upload, body sha256 must equal `:hash` → `{ok, hash, size}` or `413 artifact_too_large` (names the room's `artifact_max_bytes`) or `422 artifact_hash_mismatch`. `Content-Length` above `artifact_max_bytes` is rejected up front; the ACTUAL streamed byte count is also checked throughout and aborts mid-stream past the cap (a lying/absent header cannot force unbounded buffering). Once the ACTUAL streamed total crosses one 10 MiB part, the PUT promotes to R2 native multipart (Intent P, v1.8) and keeps flushing 10 MiB parts from there — never gated on the declared `Content-Length`; below that, a single buffered `R2.put()` closes it out |
| `POST /v1/rooms/:room/config` | owner only; body: any of `{default_access, rate_limit, authority_source, archive, fts_max_file_bytes, artifact_max_bytes}` → `202 {seq, entry_hash}`; live-retunes the running room (§3 `system.config`) |
| `WS /v1/rooms/:room/stream?since=&performative=&exclude=` | replays from `since`, then live frames (§6); `performative=`/`exclude=` (same CSV/wildcard semantics as `GET /entries`) apply to both the replay and every subsequent live frame, persisted on the connection across hibernation. Replay transparently reads through an archived range (Intent P, v1.8, mirrors `GET /entries`), capped at the same 10k-entry archived-replay budget — on cap, the connection sends one `{type:"backfill_truncated", resume_since}` frame (§6) then continues the live stream normally; the client MUST page `GET /entries` from `resume_since` up to the head it first receives, to fill the gap. Auth: `Authorization` header, or `?token=` for clients that cannot set WS headers |
| `GET /v1/rooms/:room/state` | `{head: {seq, entry_hash}, claims[], roster[], decisions[], escalations[], bindings[], defaults}` — `escalations[]` (Intent L, T6/7): the durable open/acked projection (see `escalate.ack` §3); `bindings[]` (Intent M, T13a): the SAME resolved role-binding rows `GET /roles` serves (`{participant, role, active_from, active_until, in_window}`), duplicated onto the daemon's one standing poll so every attention/duty/badge consumer reads authority from bindings — NEVER the roster's join-time card cohort (`roster.roles`; the audit §1.1 rule) — each `roster[]` row carries `condition`/`condition_seq`/`condition_ts` (Intent F; `condition_ts` additive, T11/T12): `condition` and `condition_ts` read `null` unless the participant currently holds a CLAIMED task (claim-gated, S-F6 — `condition_ts` mirrors `condition`'s gate exactly, an ISO8601 timestamp when present); `condition_seq` is unconditional (always reflects the stored fold, gate or no gate); the room never judges `condition_ts` stale, only threads it — staleness is a view-side annotation (T12); each `decisions[]` row (Intent H, v1.13.0) carries `id, question, refs, authority, deadline, fallback_note, status, resolution, resolved_by, resolved_via, request_seq, resolved_seq, lapsed_seq, task_ref` plus, only while `status === "open"`, an `authority_holders` map resolving each `role:` arm to its current time-boxed holder(s) (R-G1) — a CLI display aid, never itself an authorization check; `defaults` (v1.8, Intent P) carries the room's effective `archive:{threshold_entries}`/`fts_max_file_bytes`/`artifact_max_bytes` — always present, even before the owner ever calls `POST /config` (S-P5) |
...
| `GET /v1/rooms/:room/verify?from=&to=` | chain check helper → `{ok, head}` (clients SHOULD also verify locally). `from`/`to` default to the live tail only (never touches R2 unspecified); an explicit `from` reaching into an archived range (Intent P, v1.8) transparently chain-checks each covered R2 archive object, one range at a time, before the live tail |
| `GET /v1/rooms/:room/snapshot` | latest checkpoint `{head:{seq,entry_hash}, claims[], roster[], decisions[], room_sig}`, synthesized fresh on every read and signed with the room key — so reconnect/verify need not replay from genesis. Distinct from the DURABLE, logged `system.checkpoint` entries (Intent P, v1.8, §3/§4.1): those anchor archive-to-R2 pruning; this endpoint is always a live re-synthesis of the SAME payload shape, independent of whether any durable checkpoint has ever fired |
